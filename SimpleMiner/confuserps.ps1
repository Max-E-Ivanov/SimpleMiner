...
#echo "binDir" $BinDir
#echo "objDir" $ObjDir
$PFXPath = Split-Path (Split-Path $ObjDir -Parent) -Parent
$PFXFile = [string]::Concat($PFXPath, "\MaxCodeSigning.pfx")
echo "Signing..."
#echo  "Pfx Dir "$PfxDir
echo  "Pfx file "$PFXFile
#echo  "Exe file "$ModuleName
start-process -NoNewWindow -wait "signtool" "sign /f $PFXFile /p 12345  $ModuleName"
